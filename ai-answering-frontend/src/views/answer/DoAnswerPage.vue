...
import { questionGetDataList } from "@/api/questionController";
import message from "@arco-design/web-vue/es/message";
import { appGetData } from "@/api/appController";
import { userAnswerCreateData } from "@/api/userAnswerController";
...
const loadData = async () => {
  ...
  // 获取题目
  res = await questionGetDataList({
    searchCondition: [{ name: "appId", value: props.appId, type: "1" }],
    pageIndex: 1,
    pageSize: 1,
...
const doSubmit = async () => {
  ...
  const res = await userAnswerCreateData({
    appId: props.appId as any,
    choicesList: answerList,
  });
  if (res.data.code === 0 && res.data.data) {
    router.push(`/answer/result/${res.data.data.userAnswerId}`);
  } else {
