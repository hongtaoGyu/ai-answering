...
const loadData = async () => {
  ...
  const res = await questionGetDataList({
    searchCondition: [{ type: "1", value: props.appId, name: "appId" }],
    pageIndex: 1,
    pageSize: 1,
  ...
  if (res.data.code === 0 && res.data.data?.records) {
    ...
    if (oldQuestion.value) {
      questionContent.value =
        JSON.parse(oldQuestion.value.questionContent ?? "[]") ?? [];
    }
  } else {
...
const handleSubmit = async () => {
  ...
  let res: any;
  res = await questionSaveOrUpdateData({
    questionId: oldQuestion.value?.questionId,
    appId: props.appId as any,
    questionContentDTOList: questionContent.value,
  });
  if (res.data.code === 0) {
