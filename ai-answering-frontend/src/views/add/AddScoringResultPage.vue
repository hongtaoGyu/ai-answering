<template>
  <div id="addScoringResultPage">
    ...
    >
      ...
      <a-form-item field="resultPropList" label="结果集">
        <a-input-tag
          v-model="form.resultPropList"
          :style="{ width: '320px' }"
          allow-clear
...
import { scoringResultSaveOrUpdateData } from "@/api/scoringResultController";
...
const doUpdate = (scoringResult: API.ScoringResultVO) => {
  updateId.value = scoringResult.scoringResultId;
...
const handleSubmit = async () => {
  ...
  form.value.resultProp = JSON.stringify(form.value.resultPropList);
  let res: any;
  // 如果是修改
  res = await scoringResultSaveOrUpdateData({
    scoringResultId: updateId.value as any,
    ...form.value,
  });
  if (res.data.code === 0) {
