...
import API from "@/api";
import { appGetAppUserVo } from "@/api/appController";
...
const data = ref<API.AppUserVO>({});

// 获取登录用户
const loginUserStore = useLoginUserStore();
let loginUserId = loginUserStore.loginUser?.userId;
// 是否为本人创建
const isMy = computed(() => {
  return loginUserId && loginUserId === data.value.createUserId;
});
...
const loadData = async () => {
  ...
  const res = await appGetAppUserVo({
    id: props.id as any,
  });
  if (res.data.code === 0) {
